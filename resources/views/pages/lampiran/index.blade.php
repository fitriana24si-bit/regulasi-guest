            ...
            <div class="card kategori-card h-100">
                ...
                <div class="card-header-custom category-gradient-{{ ($loop->index % 4) + 1 }}">
                    <div class="d-flex justify-content-between align-items-start text-white">
                        ...
                        <span class="badge bg-white text-dark">{{ strtoupper($item->tipe_file) }}</span>
                    </div>

                    <h5 class="mt-2">{{ $item->nama_file }}</h5>
                ...
                <div class="card-body">
                    ...
                    {{-- PREVIEW FILE --}}
                    @if(Str::contains($item->tipe_file, ['jpg','png','jpeg']))
                        <img src="{{ asset('storage/' . $item->file_path) }}"
                             class="img-fluid rounded border mb-3"
                             style="max-height: 200px; object-fit: cover;">
                    @elseif(Str::contains($item->tipe_file, ['pdf']))
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <form action="{{ route('lampiran.destroy', $item->lampiran_id) }}"
                              method="POST"
                              class="flex-fill"
                              onsubmit="return confirm('Yakin menghapus lampiran?')">
                            @csrf
                            @method('DELETE')
